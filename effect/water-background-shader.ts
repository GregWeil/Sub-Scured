...
void main() {
  ...
  float noise2 = psrdnoise((worldPos + 0.4 * offset) * 0.03, vec2(0.0, 0.0), Time + 10.0, unused);
  float noise3 = psrdnoise((worldPos + 0.2 * offset) * 0.06, vec2(0.0, 0.0), Time + 100.0, unused);
  float noiseg = psrdnoise((worldPos + 0.3 * offset) * 0.6, vec2(0.0, 0.0), Time * 5.0, unused);
  vec3 waterColor = vec3(0.03, 0.01 + 0.05 * abs(noiseg), 0.15 + noise1 * 0.05 + noise2 * 0.02 + noise3 * 0.02) + 0.1 * vec3(vUv, 0.0);
